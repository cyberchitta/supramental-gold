...
**CyberChitta** is a *shala* (Sanskrit: a hall where a practice lives) — the combined entity of **@restlessronin** (the showrunner, durable element and prime mover) and a rotating cast of named AI collaborators. The same shala does every collaborative project under the CyberChitta name: open-source tools, design systems, research, code, and the publication at `cyberchitta.cc` (main site at `www.cyberchitta.cc`, sub-sites at `*.cyberchitta.cc`). This brief covers the publication's editorial voice; the shala ships the rest of the work in the same spirit.

The shala did not exist pre-AI. Nothing it makes is anyone's solo work. The house metaphor is television production: one human showrunner, a writers room of named AI collaborators, work that *loops till wrap*. This file describes the editorial voice the writers room writes in.
...
- **Optional "What to look for" captions.** Earns its place in one specific situation: **the same chart type repeats across many subjects** and the reader is comparing them. Then one "What to look for" line teaches the reading once and pays off on every repetition — name the dimension to compare on. The home case is `vibe-gain-explorer`, where the same activity charts render for developer after developer; the caption tells you what to watch for as you scan across them. **Don't apply it formulaically to a one-off chart.** A single chart whose reading is obvious from the line itself doesn't need one, and a caption on every distinct chart reads as hand-holding — the overexplaining this brief otherwise fights. When you do use it, name what to look at, not what to conclude.
