...
description: Router for the CyberChitta design system and house craft — the studio of @restlessronin (Showrunner) and named AI collaborators, whose publication runs at cyberchitta.cc. Seven task-specific child skills (design-throwaway, design-surface, wire-consumer, wire-deploy, draft-article, copyedit, vidhi) and two craft briefs (voice.md, visual.md). Use this skill if you're not sure which child fits.
user-invocable: true
---
...
| Skill | Use when |
|---|---|
| **`vidhi`** | Authoring or restructuring a Claude skill in any CyberChitta repo — anatomy conventions, structural splits (router/child, engine/domain-pack), and the mandatory feedback loop every skill ships with. |
...
This applies to all seven children and to the router itself. If a child's hand-off contract is wrong, log it. If the router sends you to the wrong child, log it.

## If invoked without guidance

Ask the user one question:

> "Are you (a) producing a one-off artifact, (b) doing production design on an existing CyberChitta site, (c) setting up a new consumer site, (d) drafting a new article from scratch, (e) editing an existing draft, or (f) writing or restructuring a skill?"

Then hand off to `design-throwaway`, `design-surface`, `wire-consumer`, `draft-article`, `copyedit`, or `vidhi` respectively.
