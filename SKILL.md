...
| Skill | Use when |
|---|---|
| **`vibe`** | Writing or reviewing code in any CyberChitta repo — universal house code style (functional, immutable, natural failure over validation) plus per-language references (Python, JavaScript, Jupyter). Ported from the llm-context chat-era rules. |
...
This applies to all eight children and to the router itself. If a child's hand-off contract is wrong, log it. If the router sends you to the wrong child, log it.

## If invoked without guidance

Ask the user one question:

> "Are you (a) producing a one-off artifact, (b) doing production design on an existing CyberChitta site, (c) setting up a new consumer site, (d) drafting a new article from scratch, (e) editing an existing draft, (f) writing code, or (g) writing or restructuring a skill?"

Then hand off to `design-throwaway`, `design-surface`, `wire-consumer`, `draft-article`, `copyedit`, `vibe`, or `vidhi` respectively.
